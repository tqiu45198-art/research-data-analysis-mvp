...
# ---------------------- 2. 工具函数（缓存、数据处理、图表生成） ----------------------
@st.cache_data(show_spinner="加载数据中...")  # 数据缓存，避免重复加载
def load_and_clean_data(file):
    """加载文件并清理列名（兼容 CSV/Excel）"""
...
def generate_multiple_charts(analysis_type, params, df):
    """根据分析类型生成多种图表（Plotly+Altair+Matplotlib）"""
    charts = {}
    ...
    # 6. 地理分布（地图点图）
        ...
        # Streamlit 原生地图（带点大小映射）
        df_geo = df[[lon_col, lat_col, value_col]].dropna()
        df_geo.columns = ['lon', 'lat', 'value']  # 适配 st.map 格式
...
if 'df' in locals():
    ...
    analysis_key = analysis_type.split(".")[0].strip()  # 提取分析类型标识
    ...
    if st.button("🚀 开始分析", type="primary"):
        with st.spinner("分析中..."):
            ...
            # 其他分析类型（回归、聚类、时间序列等）逻辑类似，此处省略（完整代码已包含）
            st.divider()
